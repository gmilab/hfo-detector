classdef hfodat
  ...
  methods
    ...
    function hfodata = run_ripplelab(obj, ch_idx, method)
      ...
      addpath(fullfile(fileparts(mfilename('fullpath')), 'RIPPLELAB'));
    ...
    function hfodata = run_hscdetector(obj, varargin)
      ...
      hfodata.trigs = cell(length(ch_idx), 1);
      for kk = 1:length(ch_idx)
        hfodata.trigs{kk} = ieeg_hfodetector_boundingbox(data_cnt, ch_idx(kk));
      end
      hfodata.channels = ch_idx;
    ...
    function confusion_data = run_all(obj, varargin)
      ip = inputParser;
  ...
  methods(Static)
    ...
    function obj = load_donos()
      ...
      for kk = 1:2
        ...
        load(fullfile(fileparts(mfilename('fullpath')), '..', 'DonosDetector', 'Datasets',...
